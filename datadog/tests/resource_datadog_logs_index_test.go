...
func TestAccDatadogLogsIndex_Basic(t *testing.T) {
	if !isReplaying() {
		// Skip in non replaying mode, since the API takes a while to be
		// consistent. If you really need to record the interactions, comment the
		// following return statement, and run locally.
